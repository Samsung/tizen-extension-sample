...
void request_user_code(appdata_s* ad)
{
	...
	CURL *curl;
	CURLcode res;
	char *proxy_address;// = proxy_addr;
	connection_h connection;
	res=connection_create(&connection);
	dlog_print(DLOG_INFO, LOG_TAG, "connection_create failed with %d", res);

	sprintf(request, "client_id=%s&scope=email profile", CLIENT_ID);
	...
	const char* device_code = json_object_get_string_member(json_object_root_node, "device_code");
	const char* user_code = json_object_get_string_member(json_object_root_node, "user_code");
	const char* verification_url = json_object_get_string_member(json_object_root_node, "verification_url");
	...
	dlog_print(DLOG_INFO, LOG_TAG, "device_code = %s", device_code);
	dlog_print(DLOG_INFO, LOG_TAG, "user_code = %s", user_code);
	dlog_print(DLOG_INFO, LOG_TAG, "verification_url = %s", verification_url);
	...
	strncpy(ad->user_code_response->device_code, device_code, strlen(device_code));
	strncpy(ad->user_code_response->user_code, user_code, strlen(user_code));
	strncpy(ad->user_code_response->verification_url, verification_url, strlen(verification_url));
	...
	app_control_set_uri(app_control, "https://www.google.com/device");
...
int request_access_token(appdata_s* ad)
{
	...
	sprintf(request, "client_id=%s&client_secret=%s&code=%s&grant_type=http://oauth.net/grant_type/device/1.0", CLIENT_ID, CLIENT_SECRET, ad->user_code_response->device_code);
	dlog_print(DLOG_INFO, LOG_TAG, "request params = %s", request);
...
int request_profile(appdata_s* ad)
{
	...
	sprintf(request, "https://www.googleapis.com/plus/v1/people/me?access_token=%s", ad->access_token_response->access_token);
	dlog_print(DLOG_INFO, LOG_TAG, "request params = %s", request);
	...
	const char* name = json_object_get_string_member(json_object_root_node,  "displayName");

	JsonArray* emails_array = json_object_get_array_member(json_object_root_node, "emails");
	int array_len = json_array_get_length(emails_array);

	printf("array_len = %d\narray_len", array_len);

	dlog_print(DLOG_INFO, LOG_TAG, "name = %s", name);

	JsonNode* email_node = json_array_get_element(emails_array, 0);
	JsonObject* email_obj = json_node_get_object(email_node);

	const char* email = json_object_get_string_member(email_obj,  "value");
...
{
	dlog_print(DLOG_INFO, LOG_TAG, "request_access_token_cb");
...
static void _onButtonClickedCb(void *data, Evas_Object *obj, void *event_info)
{
	...
	sprintf(buffer, "user_code :\n %s", ad->user_code_response->user_code);
